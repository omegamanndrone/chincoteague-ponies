...
✅ **Phase 2 — DEPLOYED LIVE 2026-06-13. Now in a 1–2 week soft-launch bug-hunt with Kristina.** All steps below shipped; the live site serves 236 ponies and her device auto-updates via the service worker (contentVersion 2, local data preserved — she backed up first). **Decision: the scrape needs no Kristina review gate** — the website is authoritative, validation was clean, the departed-gate was empty, and don't-clobber protects her richer book values; the console review is reserved for the INGEST (her photos/bands). Confirmed with user: scraped data lands in a new pedigree-keyed `scraped_horses` table overlaid at build (not by altering the book table); **sex ships mare/stallion** (build maps the website's female/male); pre-merge DB backed up at `tools/curator/out/backups/2026-06-13_pre-scrape-merge/`.
...
5. ✅ **App side — SHIPPED.** `horse.dart` `background` (dropped `bookInfo`); `horse_detail_screen.dart` rebuilt to the ID card + refined with the user (consistent caps; region only on the herd button — not duplicated; M/B/F/H+registry as a quiet small-caps line, not badges; uniform Description rows; age "(b. YYYY)"); sire/dam tappable when the parent is in canon (else plain — see [[pedigree-expansion-todo]] for the future multi-gen view). **Chincoteague/Assateague island switch** = tappable app-bar title (persisted, herds never mix, hidden until MD ships) + VA-only N/S region sub-filter + Assign Herd; MD region UI gated off. `_bundledContentVersion`→2 (reload canon, preserve local boxes) + a content-update test proving local data survives. `build_assets --out ../../horse_app/assets` → `flutter build web --base-href /chincoteague-ponies/` → pushed to gh-pages → verified live serving 236. Markings get a dedicated filter UI in Phase 3 (text search already matches them today).
...
- `horse_detail_screen.dart` — **redesigned as ONE continuous "ID card" (hybrid: glanceable, with collapsible low-value tiers), decided 2026-06-12.** Top→bottom:
  ...
  - **BANDS (always open, prominent — VA only):** current band + history (last 2 yrs) — Kristina's editable observational data, provenance-accented. **VA-only, like `region`** (decided 2026-06-13): the wild Chincoteague ponies band; the MD/Assateague herd is more domesticated and isn't observed banding, so the card hides Bands for `state=='MD'` and the VA band picker excludes MD ponies. (Revisit if MD is observed banding.)
